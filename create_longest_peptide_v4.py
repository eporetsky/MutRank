...
records = list(SeqIO.parse("200304_Zea_mays.AGPv4.pep.all.fa", "fasta"))
...
while i <= len(records)-1:
    ...
    if name[0]=="G":
    	...
    	gene_names.add(name[:14]) # I'm note shortening their names
    else:
...
with open("200304_Zea_mays.AGPv4.pep.all.longest.fa","w") as f:
	for seq_record in SeqIO.parse("200304_Zea_mays.AGPv4.pep.all.fa", "fasta"):
		try:
